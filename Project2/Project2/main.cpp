...
int enemy_level(Team team,Pet* enemy_pet,int player_id,int r_potion,int a_potion);
...
void play(Team& team, std::istream& is, std::ostream& os)
{
    ...
    while(is)
    {
        ...
        int enemy_action_level = enemy_level(team,enemy_pet,player_id,r_potion,a_potion);
        int action_num;
...
int enemy_level(Team team,Pet* enemy_pet,int player_id,int r_potion,int a_potion)
{
    ...
    if(enemy_id == 1 && enemy_pet->get_hp() < 55)
    {
        level = 6;
    }
    else
...
int enemy_skill(std::ostream& os,Team team,Pet* enemy_pet,Pet* player_pet,int player_id,Weather weather)
{
    ...
    if(enemy_id == 5)
    {
        if(hp < (hp_max/3)&&(player_pet->is_asleep()||player_pet->whether_painted()))
        {
            return 0;
        }
        else if(!player_pet->is_asleep())
        {
            return 1;
        }
        else if(!player_pet->whether_painted())
        {
            return 2;
        }
        else
    ...
    {
        if ((enemy_hp > enemy_hp_max/2) || (enemy_hp > hp))
        {
            if(id == 3)
            {
                return 1;
    ...
    {
        if(hp < hp_max/5 &&(id == 3 || id == 2))
        {
            return 0;
        }
        else if(weather.get_type() == sunny)
        {
            return 1;
        }
        else
