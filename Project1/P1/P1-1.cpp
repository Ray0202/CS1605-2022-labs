...
void enemy_action(std::ostream& os,Team team,Pet* enemy_pet,Pet* player_pet,int player_id);
void enemy_change(std::ostream& os,Pet*& enemy_pet,Team team,Pet* player_pet);
    ...
    while(is)
    {
        ...
        if(player_action_level >= enemy_action_level)
        {
           switch(action_num)
            ...
            enemy_action(os,team,enemy_pet,player_pet,player_id);
        }
        ...
        {
            enemy_change(os, enemy_pet, team, player_pet);
            os << "You Lose" << endl;
            break;
        }
        else if(flag == 2)
